...
`run-card`, `run-diff`, `run-trace`, `pr-affordance`, `card-drawer`
...
| 2 | Worker exits non-zero before SDK loop | `Supervisor.attachLifecycle` exit handler when `!active.ready` | `rejectReady` rejects the `startRun` promise; the route returns 500 with the rejection message; UI shows "Run failed to start" in the drawer or board card | ErrorCard with Retry + Copy details (drawer + run-card + board-card) | Drawer previously showed plain amber text. | **Fixed**: drawer now renders an `<ErrorCard />` with Retry and Copy details when `actuallyRun()` fails; matches `run-card` and `board-card`. |
...
| 8 | `repoPath` not a git repo | `git worktree add` failure in `src/worker/git.ts` | Worker emits `error` wire message with the `git` stderr; `RunLog` shows `[error]` red row; run ends `failed` | `[error]` log row | None. | Already correct. Copy details is available on the surrounding run-card / drawer ErrorCard. |
